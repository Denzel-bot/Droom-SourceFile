...
public class portalSetup : MonoBehaviour {
    ...
    void Start () {
        ...
        camera1.transform.SetParent(containerCamA.transform, true);
        camera2.transform.SetParent(containerCamB.transform, true);
        ...
        //and its scripts
        portalCamMovement scriptCamA = camera1.gameObject.AddComponent<portalCamMovement>();
        portalCamMovement scriptCamB = camera2.gameObject.AddComponent<portalCamMovement>();

        //I give this new cameras same setup than main camera
        Camera cameraComp = playerCamera.GetComponent<Camera>();
        camera1.CopyFrom(cameraComp);
        camera2.CopyFrom(cameraComp);
    ...
    void Resize(bool fireEvent) {
        ...
        camera1.targetTexture = new RenderTexture(Screen.width, Screen.height, 0);
        matA.mainTexture = camera1.targetTexture;
        matA.SetTexture("_MainTex", camera1.targetTexture);

        Material matB = new Material(refShader);
        camera2.targetTexture?.Release();
        camera2.targetTexture = new RenderTexture(Screen.width, Screen.height, 0);
        matB.mainTexture = camera2.targetTexture;
